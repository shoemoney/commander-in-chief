	...
	for i in sim.players.size():
		...
		if p["alive"]:
			...
			if bt > 0:
				...
				# Measured at the SAME resolved size it is drawn at (accessibility TEXT SIZE), or
				# the centering offset would drift by half the scale-up.
				var brw := Art.font().get_string_size(btxt, HORIZONTAL_ALIGNMENT_LEFT, -1, Art.fs(8)).x
