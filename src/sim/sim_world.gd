...
func _kill_enemy(e: Dictionary, no_coin := false) -> void:
	...
	if e["elite"] and e["kind"] != "mg_nest" and not no_coin:
		# The rare capsule: usually Piercing/Spread (timed buffs); Triple Shot is a
		# PERMANENT mod, so it drops far rarer (a ~1-in-3 promote of the rare roll).
		var cap_kind := rng.range_i(0, 1)   # 0=Ammo, 1=Grenade
		if rng.range_i(0, 5) == 0:          # ~1-in-6 elites drop a power-up capsule
			cap_kind = 4 + rng.range_i(0, 1)   # 4=Piercing, 5=Spread
			if rng.range_i(0, 2) == 0:
				cap_kind = 6                    # ...rarely promoted to the permanent Triple
		pickups.append({"x": e["x"], "y": e["y"], "kind": cap_kind})
