...
func _step_enemies() -> void:
	...
	for i in range(enemies.size() - 1, -1, -1):
		var e := enemies[i]
		# Rally masts are exempt from the off-screen sweep in the fixed-camera
		# modes (nothing ever leaves the band there, and a mast is a standing
		# objective). The CAMPAIGN camera ratchets, so a mast left behind would
		# never be culled — every one you walked past would live in enemies[] and
		# in the _broadcasts aura scan for the rest of the run. Sweep it there.
		if not e["alive"] or (e["y"] > camera_top + 420 * F_ONE
				and (e["kind"] != "broadcast" or mode == "campaign")):
			enemies.remove_at(i)
			continue
		if flash_ticks > 0:
